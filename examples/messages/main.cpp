...
int main() {
  ...
  // // cout << "c: " << c.linear_acceration.x << " " << c.temperature << " " <<
  // c.timestamp << endl; printf("Raw IMU (%lu):\n", sizeof(c)); printf("
  // header: %u %llu %d:\n", c.type, c.timestamp, c.error);
  // // printf(" a: %f %f %f\n", c.linear_acceration.x, c.linear_acceration.y,
  // c.linear_acceration.z);
  // // printf(" g: %f %f %f\n", c.angular_velocity.x, c.angular_velocity.y,
  // c.angular_velocity.z);
  // // printf(" m: %f %f %f\n", c.magnetic_field.x, c.magnetic_field.y,
  // c.magnetic_field.z); printf(" t: %f\n", c.temperature);
