...
bool Stream::open(const std::string &port, int speed) {
  ...
  // port. fd = ::open(port.c_str(), O_RDWR|O_NOCTTY|O_NONBLOCK); guard(fd =
  // ::open(port.c_str(), O_RDWR|O_NOCTTY), "open(): ");
  ...
  // if (tcsetattr(fd, TCSANOW, &t) < 0) perror("*** Couldn't set port
  // attribute");
...
// Flush input buffer
void Stream::flush_input() { guard(tcflush(fd, TCIFLUSH), "flush_input(): "); }
